...
uint32_t nrf24_get_rate()
{
    uint8_t setup = 0;
    uint32_t rate = 0;
    nrf24_read_reg(REG_RF_SETUP, &setup, 1);
    setup &= 0x28;
    if(setup == 0x20)
        rate = 250000; // 250kbps
    else if(setup == 0x08)
...
uint8_t nrf24_set_src_mac(uint8_t* mac, uint8_t size)
{
    ...
    nrf24_write_buf_reg(REG_RX_ADDR_P0, clearmac, 5);
...
uint8_t nrf24_set_dst_mac(uint8_t* mac, uint8_t size)
{
    ...
    uint8_t clearmac[] = {0, 0, 0, 0, 0};
    nrf24_set_maclen(size);
    nrf24_write_buf_reg(REG_TX_ADDR, clearmac, 5);
...
uint8_t nrf24_rxpacket(uint8_t* packet, uint8_t* ret_packetsize, uint8_t packet_size)
{
    ...
    {
        if(packet_size == 1)
            packet_size = nrf24_get_packetlen((status >> 1) & 7);
        ...
        if(packet_size > 32 || packet_size == 0)
            packet_size = 32;
...
uint8_t nrf24_set_rx_mode()
{
    uint8_t status = 0;
    uint8_t cfg = 0;
    //status = nrf24_write_reg(REG_CONFIG, 0x0F); // enable 2-byte CRC, PWR_UP, and PRIM_RX
    nrf24_read_reg(REG_CONFIG, &cfg, 1);
...
uint8_t nrf24_set_tx_mode()
{
    ...
    uint8_t cfg = 0;
    furi_hal_gpio_write(nrf24_CE_PIN, false);
    nrf24_write_reg(REG_STATUS, 0x30);
...
void nrf24_configure(uint8_t rate, uint8_t* srcmac, uint8_t* dstmac, uint8_t maclen, uint8_t channel, bool noack, bool disable_aa)
{
    ...
    if(disable_aa)
        ...
        nrf24_write_reg(REG_EN_AA, 0x1F); // Enable Shockburst
    ...
    if(maclen)
        nrf24_set_maclen(maclen);

    if(srcmac)
        nrf24_set_src_mac(srcmac, maclen);
...
uint8_t nrf24_set_mac(uint8_t mac_addr, uint8_t *mac, uint8_t mlen)
{
	...
	return nrf24_write_buf_reg(mac_addr, addr, mlen);
}